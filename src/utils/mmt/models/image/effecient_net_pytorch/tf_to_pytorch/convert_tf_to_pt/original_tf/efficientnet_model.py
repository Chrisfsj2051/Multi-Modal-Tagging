    ...
    def _build(self):
        ...
        if self._block_args.condconv:
            ...
            self._avg_pooling = tf.keras.layers.GlobalAveragePooling2D(
                data_format=self._data_format)
            self._routing_fn = tf.layers.Dense(self._condconv_num_experts,
...
class MBConvBlockWithoutDepthwise(MBConvBlock):
    """MBConv-like block without depthwise convolution and squeeze-and-
    excite."""
    def _build(self):
        """Builds block according to the arguments."""
        filters = self._block_args.input_filters * self._block_args.expand_ratio
...
class Model(tf.keras.Model):
    ...
    def _build(self):
        ...
        for block_args in self._blocks_args:
            ...
            if not block_args.super_pixel:  #  no super_pixel at src
                self._blocks.append(conv_block(block_args,
                                               self._global_params))
            else:
