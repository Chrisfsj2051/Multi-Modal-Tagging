...
def count_func_time(func):
    def call_fun(*args, **kwargs):
        ...
        func(*args, **kwargs)
        end_time = time.time()
        print('{} cost {:.3f} sec'.format(func.__name__,
                                          end_time - start_time))
...
class PRCalculatorPerTag():
    ...
    def accumulate(self, predictions, actuals):
        ...
        for i in range(self.tag_num):
            self.pr_calculators[i].accumulate(pred_per_tag_list[i],
                                              actuals_per_tag_list[i])
        # ret = list(map(map_func, self.pr_calculators, pred_per_tag_list, actuals_per_tag_list))

    def get_precision_list(self, th=0.5):
        return [
            self.pr_calculators[i].get_precision_at_conf(th)
            for i in range(self.tag_num)
        ]

    def get_recall_list(self, th=0.5):
        return [
            self.pr_calculators[i].get_recall_at_conf(th)
            for i in range(self.tag_num)
        ]
