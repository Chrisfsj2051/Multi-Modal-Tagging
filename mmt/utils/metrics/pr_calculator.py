...
class PRCalculator():
    def __init__(self):
        ...
        self.threshold_dict = {
            0.5: 0,
            0.1: 1
        }  # TODO(jefxiong, range from 0.9~0.01)
    ...
    def accumulate(self, predictions, actuals):
        ...
        for i in range(n_example):
            ...
            for th, th_index in self.threshold_dict.items():
                ...
                precision_all[i][th_index] = tp * 1.0 / len(pred_index) if len(
                    pred_index) > 0 else np.nan
                recall_all[i][th_index] = tp * 1.0 / len(gt_index) if len(
                    gt_index) > 0 else np.nan

        valid_accumlate = (np.sum(~np.isnan(precision_all), axis=0)) != 0
        self.accumulate_count = self.accumulate_count + valid_accumlate

        precision_all = np.nansum(precision_all, axis=0) / (
            np.sum(~np.isnan(precision_all), axis=0) + 1e-10)
        recall_all = np.nansum(recall_all, axis=0) / (
            np.sum(~np.isnan(recall_all), axis=0) + 1e-10)
