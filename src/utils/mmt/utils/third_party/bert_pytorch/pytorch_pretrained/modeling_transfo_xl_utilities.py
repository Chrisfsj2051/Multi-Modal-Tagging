...
class ProjectedAdaptiveLogSoftmax(nn.Module):
    ...
    def forward(self, hidden, target=None, keep_order=False):
        '''
            ...
            if their's had an option to set bias on src clusters in the native one.
    ...
    def log_prob(self, hidden):
        r""" Computes log probabilities for src :math:`n\_classes`
...
if __name__ == '__main__':
    ...
    # assert torch.src(torch.sort(sample.unique())[0].eq(torch.sort(sample)[0])).item()
