...
def build_learning_rate(initial_lr,
    ...
    if warmup_epochs:
        ...
        warmup_lr = (initial_lr * tf.cast(global_step, tf.float32) /
...
def archive_ckpt(ckpt_eval, ckpt_objective, ckpt_path):
    ...
    if saved_objective > ckpt_objective:
        logging.info('Ckpt %s is worse than %s', ckpt_objective,
                     saved_objective)
        return False
...
def get_ema_vars():
    """Get src exponential moving average (ema) variables."""
...
class EvalCkptDriver(object):
    ...
    def build_dataset(self, filenames, labels, is_training):
        ...
        if 'condconv' in self.model_name and not is_training:
            ...
            # must drop src remaining examples that do not make up one full batch.
            # To ensure src examples are evaluated, use a batch size that evenly
            # divides the number of files.
            batch_drop_remainder = True
            num_files = len(filenames)
            if num_files % self.batch_size != 0:
                tf.logging.warn(
                    'Remaining examples in last batch are not being '
                    'evaluated.')
    ...
    def eval_example_images(self,
        ...
        pred_idx, pred_prob = self.run_inference(ckpt_dir, image_files,
                                                 [0] * len(image_files),
    ...
    def eval_imagenet(self, ckpt_dir, imagenet_eval_glob, imagenet_eval_label,
                      num_images, enable_ema, export_ckpt):
        """Eval ImageNet images and report top1/top5 accuracy.

        Args:
          ckpt_dir: str. Checkpoint directory path.
          imagenet_eval_glob: str. File path glob for src eval images.
        ...
        """
        imagenet_val_labels = [
            int(i) for i in tf.gfile.GFile(imagenet_eval_label)
        ]
