...
def read_metadata(csv_path, classes_num, id_to_ix):
    ...
    for n, line in enumerate(lines):
        ...
        audio_name = 'Y{}.wav'.format(
            items[0])  # Audios are started with an extra 'Y' when downloading
...
def pad_or_truncate(x, audio_length):
    """Pad src audio to specific length."""
...
class StatisticsContainer(object):
    ...
    def dump(self):
        ...
        pickle.dump(self.statistics_dict,
                    open(self.backup_statistics_path, 'wb'))
        logging.info('    Dump statistics to {}'.format(self.statistics_path))
        logging.info('    Dump statistics to {}'.format(
