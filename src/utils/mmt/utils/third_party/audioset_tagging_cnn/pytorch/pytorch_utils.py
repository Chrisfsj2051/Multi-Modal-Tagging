...
def forward(model, generator, return_input=False, return_target=False):
    ...
    for n, batch_data_dict in enumerate(generator):
        ...
        if 'segmentwise_output' in batch_output.keys():
            append_to_dict(
                output_dict, 'segmentwise_output',
...
def count_flops(model, audio_length):
    ...
    def foo(net):
        ...
        if not childrens:
            ...
            elif isinstance(net, nn.BatchNorm2d) or isinstance(
                    net, nn.BatchNorm1d):
                net.register_forward_hook(bn_hook)
            elif isinstance(net, nn.ReLU):
                net.register_forward_hook(relu_hook)
            elif isinstance(net, nn.AvgPool2d) or isinstance(
                    net, nn.MaxPool2d):
                net.register_forward_hook(pooling2d_hook)
            elif isinstance(net, nn.AvgPool1d) or isinstance(
                    net, nn.MaxPool1d):
