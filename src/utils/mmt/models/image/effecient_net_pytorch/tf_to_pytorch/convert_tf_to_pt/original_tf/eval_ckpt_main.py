...
flags.DEFINE_integer('num_images', 5000,
                     'Number of images to eval. Use -1 to eval src images.')
FLAGS = flags.FLAGS
...
class EvalCkptDriver(object):
    ...
    def restore_model(self, sess, ckpt_dir):
        ...
        ) + tf.compat.v1.get_collection('moving_vars')
    ...
    def build_model(self, features, is_training):
        ...
        features /= tf.constant(STDDEV_RGB,
                                shape=[1, 1, 3],
                                dtype=features.dtype)
        logits, _ = efficientnet_builder.build_model(features, self.model_name,
                                                     is_training)
...
def eval_imagenet(model_name, ckpt_dir, imagenet_eval_glob,
                  imagenet_eval_label, num_images):
...
def main(unused_argv):
    ...
    if FLAGS.runmode == 'examples':
        ...
        eval_example_images(FLAGS.model_name, FLAGS.ckpt_dir,
                            [FLAGS.example_img], FLAGS.labels_map_file)
    elif FLAGS.runmode == 'imagenet':
        # Run inference for imagenet.
        eval_imagenet(FLAGS.model_name, FLAGS.ckpt_dir,
                      FLAGS.imagenet_eval_glob, FLAGS.imagenet_eval_label,
                      FLAGS.num_images)
    else:
