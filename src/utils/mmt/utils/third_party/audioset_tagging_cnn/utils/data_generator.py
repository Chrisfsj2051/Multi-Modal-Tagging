...
class AudioSetDataset(object):
    def __init__(self, sample_rate=32000):
        """This class takes the meta of an audio clip as input, and return the
        waveform and target of the audio clip.
...
class Base(object):
    def __init__(self, indexes_hdf5_path, batch_size, black_list_csv,
                 random_seed):
        ...
        logging.info('Black list samples: {}'.format(len(
            self.black_list_names)))
        ...
        with h5py.File(indexes_hdf5_path, 'r') as hf:
            ...
            self.hdf5_paths = [
                hdf5_path.decode() for hdf5_path in hf['hdf5_path'][:]
            ]
        ...
        logging.info('Load target time: {:.3f} s'.format(time.time() -
                                                         load_time))
        ...
        while True:
            ...
            while i < batch_size:
                ...
                if self.audio_names[index] in self.black_list_names:
                    ...
                    batch_meta.append({
                        'hdf5_path':
                        self.hdf5_paths[index],
                        'index_in_hdf5':
                        self.indexes_in_hdf5[index]
                    })
                    i += 1
...
class BalancedTrainSampler(Base):
    def __init__(self,
        ...
        Args:
          ...
          random_seed: int
        """
        super(BalancedTrainSampler,
              self).__init__(indexes_hdf5_path, batch_size, black_list_csv,
                             random_seed)
        ...
        # Training indexes of src sound classes. E.g.:
    ...
    def __iter__(self):
        ...
        while True:
            ...
            while i < batch_size:
                ...
                if self.pointers_of_classes[
                        class_id] >= self.samples_num_per_class[class_id]:
...
class AlternateTrainSampler(Base):
    def __init__(self,
        ...
        AlternateSampler alternately sample data from Sampler and Blanced
        Sampler.
            ...
            if self.count % 2 == 0:
                ...
                while i < batch_size:
                    ...
                    if self.sampler1.pointer >= self.sampler1.audios_num:
                        ...
                        self.sampler1.random_state.shuffle(
                            self.sampler1.indexes)

                    # If audio in black list then continue
                    if self.sampler1.audio_names[
                            index] in self.sampler1.black_list_names:
                        continue
                    else:
                        batch_meta.append({
                            'hdf5_path':
                            self.sampler1.hdf5_paths[index],
                            'index_in_hdf5':
                            self.sampler1.indexes_in_hdf5[index]
                        })
                        i += 1
                ...
                while i < batch_size:
                    ...
                    # When finish one epoch of a sound class, then shuffle its indexes and reset pointer
                    if self.sampler2.pointers_of_classes[
                            class_id] >= self.sampler2.samples_num_per_class[
                                class_id]:
                    ...
                    # If audio in black list then continue
                    if self.sampler2.audio_names[
                            index] in self.sampler2.black_list_names:
                        continue
                    else:
                        batch_meta.append({
                            'hdf5_path':
                            self.sampler2.hdf5_paths[index],
                            'index_in_hdf5':
                            self.sampler2.indexes_in_hdf5[index]
                        })
                        i += 1
